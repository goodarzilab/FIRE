...
sub new {
    ...
    $self->{BLAST_DIR}        = "/Users/hani/Life/Projects/Applications/ncbi-blast-2.2.26+/bin";
    $self->{DATABASE}         = undef;
    $self->{QUERY}            = undef;
    $self->{VERBOSE}          = 0;
...
sub format {
    ...
    my $e = "perl $self->{BLAST_DIR}/legacy_blast.pl formatdb -i $f -p $type -o T";
    print "$e\n" if ($self->{VERBOSE} == 1);
...
sub blastallUnique {
    ...
    my $e = "perl $self->{BLAST_DIR}/legacy_blast.pl blastall --path $self->{BLAST_DIR} -p $self->{BLAST_PROGRAM}/$self->{BLAST_PROGRAM} -i \"$q_file\" -d \"$d_file\" -v 1 -b 1 -m 7";
...
sub blastallMultiple {
    ...
    my $e = "perl $self->{BLAST_DIR}/legacy_blast.pl blastall --path $self->{BLAST_DIR} -p $self->{BLAST_PROGRAM} -i \"$q_file\" -d $self->{DATABASE} -m 7 ";
