...
if (Sets::exist_parameter(\@ARGV, "-includenotsignif") == 1) {
  $includenotsignif     = Sets::get_parameter(\@ARGV, "-includenotsignif");
}
#
...
if ($includenotsignif == 1) {
	print 'HELLO!', "\n";
	#push @SIGSETS, "NOT-SIGNIF";
	$minrobustness = -1;
}
...
if (defined($expfile_b_dna) || defined($expfile_b_rna)) {
  ...
  if (defined($expfile_b_dna) && -s $expfile_b_dna) {
    ...
    print "HI!";
  if (defined($expfile_b_rna) && -s $expfile_b_rna) {
