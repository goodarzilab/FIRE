...
int main(int argc, char ** argv)
{
  ...
  int*         masked;
  int          max_nb_kmers = 100000, inc_nb_kmers=10000;
  int          add3 = 1;
  int          add5 = 1;
  int          add  = 2;
  int          kmersize_add;
  ...
  if (exist_parameter(argc, argv, "-maximize-freq")) 
    MAXFREQ = atoi(get_parameter(argc, argv, "-maximize-freq"));
  ...
  while ( (seq = nextSequence(fp, &name, &size, &nextSequence_started, &nextSequence_ended, nextSequence_currentLine)) ) {   
    ...
    if (true_idx > max_seq) {
      printf("non-sense: true_idx=%d > max_seq=%d (name = %s)\n", true_idx, max_seq, realname);
    }
  ...
  for (nk=0; nk<nbkmers; nk++) {
	...
	for (k=0; k<kmersize_add; k++) {
      ...
      if ((mymi > 0.75*best_mymi) && (myfreq > lastmyfreq) && (pass_cond_test == 1) && (MAXFREQ == 1)) {
	    ...
	    }else if ((mymi > best_mymi) && ((myfreq < maxfreq) || (myfreq < lastmyfreq)) && (pass_cond_test == 1)) {
        best_mymi   = mymi;
        best_l      =  l;
        best_ch_str = strdup(ch_str);
        best_count  = kmer_count;
