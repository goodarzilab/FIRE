...
int main(int argc, char ** argv)
{
  ...
  nextSequence_currentLine = (char*)malloc(500000 * sizeof(char));
  nborfs  = -1;
  realname = (char*)calloc(100, sizeof(char));

  count_seq       = -1;
  count_seq_index = (char*)calloc(1000000,  sizeof(char));
  while ( (seq = nextSequence(fp, &name, &size, &nextSequence_started, &nextSequence_ended, nextSequence_currentLine)) ) {   
     
    count_seq ++; if (count_seq == 1000000) die("pb with count_seq > 100000\n");
    //
  ...
  printf("top 10 seeds...\n") ;
  for (i=0 ; i<10 ; i++){
    printf("%s\n", getGappedKmer(kmers[kmers_array[i].index], gap));
  }
  fflush(stdout);
  ...
  if (fastthreshold == 1) {
    ...
    printf("Searches for 5 consecutive 'not passed'.\n");
    ...
    i             = min(nbkmers-1, last_idx_eval + 5);
	  
    while ((i < nbkmers) && (nb_prev_bad < 5)) {
	...
	// jump 5 down (+1 because the current one is good)
	i += 5 + 1; // add 1 because 1 is going to be removed immediately below
