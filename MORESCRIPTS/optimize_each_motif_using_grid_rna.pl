...
if (@ARGV == 0) {
  die "Usage: perl optimize_each_motif_using_grid.pl --expfile= --fastafile_dna=FILE --fastafile_rna=FILE\n";
}
...
my $summaryfile  = "$expfile\_FIRE/RNA/$expfile_only.summary";
...
foreach my $r (@$a_ref_m) {
	...
	my $motifcandidate = $r->{MOTIF};
	
	$motifcandidate =~ s/\./N/g;
	$motifcandidate =~ s/\[AG\]/R/g;
	$motifcandidate =~ s/\[CT\]/Y/g;
	$motifcandidate =~ s/\[GT\]/K/g;
	$motifcandidate =~ s/\[AC\]/M/g;
	$motifcandidate =~ s/\[CG\]/S/g;
	$motifcandidate =~ s/\[AT\]/W/g;
	$motifcandidate =~ s/\[CGT\]/B/g;
	$motifcandidate =~ s/\[AGT\]/D/g;
	$motifcandidate =~ s/\[ACT\]/H/g;
	$motifcandidate =~ s/\[ACG\]/V/g;
  ...
  my $todo = "$progdir/mi_optimize_motif -kmer $motifcandidate -fastafile $fastafile -quantized $quantized -expfile $expfile -verbose $verbose -add $add -rna $r->{RNA} -outfile $targetdir/$r->{SEED} -maxfreq $maxfreq "; 
  ...
  $pbs->setMemory("32768Mb");
  $pbs->setGB("32");
  
  # now just add commands (that you would normally exexute manually)
  $pbs->addCmd("cd $pwd");
  $pbs->addCmd("echo \"Optimizing $motifcandidate\"");
  $pbs->addCmd($todo);
