...
BEGIN {
	...
	if ( ( !$ENV{FIREDIR} ) || ( $ENV{FIREDIR} eq '' ) ) {
		print
"The FIREDIR environment variable is not set. Please set it using export or setenv (see FIRE tutorial online).\n";
		exit;
	}
...
my $morescriptdir = "$firedir/MORESCRIPTS";
...
my $no_neg_cor     = undef;
my $dooptimizetowm = 0;
my $dooptimizetoextend = 0;
my $includenotsignif = 0;

if ( @ARGV == 0 ) {
	die "Usage: [perl] fire.pl --expfiles=FILES --exptype=TXT --species=TXT

--oribiasonly=INT  if 0, do not filter out RNA motifs with no orientation bia.
...
GetOptions(
	...
	'no_neg_cor=s'           => \$no_neg_cor,
	'lp_t_draw=s'            => \$lp_t_draw,
	'dooptimizetowm=s'       => \$dooptimizetowm,
	'dooptimizetoextend=s' => \$dooptimizetoextend,
	'includenotsignif=s' => \$includenotsignif,
	'maximize-freq'     => \$maximize_freq,
);
...
if ( defined($species) ) {
	...
	if ( defined($seqlen_u_dna) && defined($seqlen_d_dna) ) {
		$fastafile_dna =
		  $species_data->{"fastafile_dna_p"}
		  . "$seqlen_u_dna\_$seqlen_d_dna.fa";
	...
	$fc_fasta1_dna = $species_data->{"fc_fasta1_dna"}
	  if ( defined( $species_data->{"fc_fasta1_dna"} ) );
	$fc_fasta2_dna = $species_data->{"fc_fasta2_dna"}
	  if ( defined( $species_data->{"fc_fasta2_dna"} ) );
	$fc_nbgenes_dna = $species_data->{"fc_nbgenes_dna"}
	  if ( defined( $species_data->{"fc_nbgenes_dna"} ) );

	$fc_fasta1_rna = $species_data->{"fc_fasta1_rna"}
	  if ( defined( $species_data->{"fc_fasta1_rna"} ) );
	$fc_fasta2_rna = $species_data->{"fc_fasta2_rna"}
	  if ( defined( $species_data->{"fc_fasta2_rna"} ) );
	$fc_nbgenes_rna = $species_data->{"fc_nbgenes_rna"}
	  if ( defined( $species_data->{"fc_nbgenes_rna"} ) );
...
if ( defined($random) ) {
	...
	for ( my $i = 0 ; $i < $random ; $i++ ) {
		my $cmd =
"perl $scriptdir/shuffle_column.pl $expfiles > $target_dir/$expfile_file.$i.txt";
		print "$cmd\n";
		system($cmd);
	}
...
foreach my $expfile (@$a_ref_files) {
	...
	if ( $dodna == 1 ) {
		...
		#
		die
"No DNA sequence data ($fastafile_dna does not exist). Perhaps you need to download the relevant 
...
#}
		...
		$pbs->addCmd("date");
		$pbs->addCmd(
			"echo \"DNA, remove duplicates, create $expfile_nodups_dna\"");
		...
		if ( $quantized == 0 ) {
			...
			my $cmd =
"perl $scriptdir/quantize_expression_vector.pl -expfile $expfile_nodups_dna -outfile $expfile_q_dna ";
			if ( defined($ebins) ) {
				$cmd .= " -ebins $ebins ";
			}
			if ( defined($divbins) ) {
...
"CNTM=`perl $scriptdir/count_lines.pl $seedfile_dna`; if [ \$CNTM = 0 ]; then echo \"No DNA motifs. Exiting now.\"; exit; fi"
		...
		if ( $doskipdiscovery == 1 ) {
			$pbs->addCmd(
"echo \"DNA, Step 2: skip seed optimization, use -motiffile_dna option instead.\""
			);
			die "Please provide -motiffile_dna\n" if ( !-e $motiffile_dna );
			$pbs->addCmd(
"perl $scriptdir/copy_seeds_to_optim.pl $motiffile_dna $optimfile_dna"
			);
		}

		if ( $domisignif == 1 ) {
			$pbs->addCmd(
				"echo \"DNA, Step 3: evaluation of motif significances.\"");
		...
		if ( $dobinary == 1 ) {
			...
			my $cmd =
"perl $scriptdir/mi_create_overep_expfile.pl -repfile $motifrepfile_dna -expfile $quantized_expfile_dna -quantized $quantized -outexpfile $expfile_b_dna";
			$pbs->addCmd(
				"echo \"DNA, Step 3.5: creating binary expression profiles.\"");
			$pbs->addCmd($cmd);
		...
		if ( $domidist == 1 ) {
			$pbs->addCmd(
				"echo \"DNA, Step 4: discovery of distance constraints.\"");

			my %PARAMS = (
				"expfile_b"      => $expfile_b_dna,
				"expfile"        => $expfile_nodups_dna,
				"quantized"      => $quantized,
				"fastafile"      => $fastafile_dna,
				"rna"            => $treatasrna,
		...
		if ( ( $dogoclusters == 1 ) && ( defined($goindex) ) ) {
			...
			my $cmd =
"perl $scriptdir/clusters_go_enrichment.pl --clusters=$quantized_expfile_dna --goindex=$goindex --gonames=$gonames --N=-1 --outfile=$gofile_full_dna --outgo=$gofile_dna --maxcatsize=$maxgocatsize ";
		...
		if ( ( $docons == 1 ) && ( defined($fc_fasta1_dna) ) ) {
			...
			my $cmd =
"perl $scriptdir/evaluate_motifs_conservation.pl -summaryfile $optimfile_dna -fasta1 $fc_fasta1_dna -fasta2 $fc_fasta2_dna -nbgenes $fc_nbgenes_dna -kmerfile $kmerfile_dna -outfile $consfile_dna ";
			if ( $shuffle_cons == 1 ) {
				$cmd .= " -shuffle 1";
			}
			$pbs->addCmd($cmd);
		...
		if ( $domisummarize == 1 ) {
			...
			my %PARAMS = (
				...
				"consfile_dna"   =>
				  ( defined($fc_fasta1_dna) ? $consfile_dna : undef ),
				"rna"               => $treatasrna,
				"quantized"         => $quantized,
				"minrobustness"     => $jn_t,
				"maxrobustness"     => $jn,
				...
				"includenotsignif" => $includenotsignif,
			);
		...
		if ( $dogetprofiles == 1 ) {
			...
			$pbs->addCmd(
"echo \"DNA, Step 5.6: generate profiles for motifs in summary file.\""
			);
			my $cmd =
"perl $scriptdir/generate_motif_profiles.pl -fastafile $fastafile_dna -summaryfile $summaryfile_dna -outfile $profiles_dna";
			$pbs->addCmd($cmd);
			...
			if ( defined($fc_fasta2_dna) ) {
				$pbs->addCmd(
"echo \"DNA, Step 5.6 bis: Generate profiles for $fc_fasta2_dna.\""
				);
				my $cmd =
"perl $scriptdir/generate_motif_profiles.pl -fastafile $fc_fasta2_dna -summaryfile $summaryfile_dna -outfile $orth_profiles_dna";
				$pbs->addCmd($cmd);
			}
		}
		...
		if ( $dotargets == 1 ) {
			...
			my $cmd =
"perl $scriptdir/get_motif_targets.pl --matrixfile=$fullmatrixfile_dna --profiles=$profiles_dna --expfile=$quantized_expfile_dna --outfile=$targets_dna";
			$pbs->addCmd($cmd);
		...
		if ( $dohistograms == 1 ) {
			...
			my $cmd =
" perl $scriptdir/draw_position_histogram.pl  --profiles=$profiles_dna --expfile=$quantized_expfile_dna --outdir=$histogramdir_dna --targetfile=$targets_dna --rna=$treatasrna --seqlen=$seqlen_dna --bins=10 ";
			$pbs->addCmd($cmd);
		...
		if ( $doagglo == 1 ) {
			...
			my $nbclusters = 4;
			my $todo       =
"perl $scriptdir/mi_cluster_motifs.pl -nbclusters $nbclusters -matrixfile $matrixfile_dna -clusterfile $clusterfile_dna";
			$pbs->addCmd($todo);
		...
		if ( $dogomotifs == 1 ) {
			...
			$pbs->addCmd(
"echo \"DNA, Step 5.6: generate genome-wide GO enrichments for motifs in summary file.\""
			);

			my $cmd =
"perl $scriptdir/motif_single_go_enrichment.pl --summaryfile=$summaryfile_dna --profiles=$profiles_dna --goindex=$goindex --gonames=$gonames --outgo=$gomofile_dna --outfile=$gomofile_full_dna --expfile=$quantized_expfile_dna --maxcatsize=$maxgocatsize ";
		...
		if ( $dodrawmotifmaps == 1 ) {
			...
			#
			$pbs->addCmd(
				"echo \"DNA, draw motif maps for co-localizing pairs\"");
			mkdir "$mimatrixfile_dna\_OUT" if ( !-e "$mimatrixfile_dna\_OUT" );

			my $cmd =
"perl $scriptdir/mi_draw_all_pair_motif_maps.pl --expfile=$quantized_expfile_dna --profiles=$profiles_dna --rootdir=$rootdir --fullmatrixfile=$fullmatrixfile_dna  --rna=$treatasrna --summaryfile=$summaryfile_dna ";

			if ( defined($seqlen_dna) ) {
				$cmd .= " --seqlen=$seqlen_dna ";
			}
			else {
				$cmd .= " --fastafile=$fastafile_dna ";
			}
			if ( defined($leftlabel_dna) ) {
				$cmd .= " --leftlabel=$leftlabel_dna ";
			}
			if ( defined($rightlabel_dna) ) {
				$cmd .= " --rightlabel=$rightlabel_dna ";
			}
		...
		if ( $domotifnames == 1 ) {
			...
			if ( defined($acelibrary_dna) ) {
				...
				my $cmd =
"perl $scriptdir/annotate_motifs_using_ace_library.pl --acefiles=\"$acelibrary_dna\" --summaryfile=$summaryfile_dna --namefile=$namesfile_dna --rna=$treatasrna --outfile=$namesfile_dna_tmp";
				$pbs->addCmd($cmd);
		...
		if ( ( $dodnacomp == 1 ) && ( $domidrawmatrix == 1 ) ) {
			...
			my %PARAMS = (
				...
				"namesfile"   => $namesfile_dna,
				"outfile"     => $summaryfile_dna . '.html'
			);

			$PARAMS{"clusterfile"} = $clusterfile_dna
			  if ( ( $sortmotifsbyphase == 0 ) && ( $domicombine >= 1 ) );

			my $cmd = &get_cmd_midrawmatrix( \%PARAMS );
			$pbs->addCmd($cmd);

			my $cmd = &get_cmd_midrawmatrix_html( \%PARAMS );
			$pbs->addCmd($cmd);
			...
			$cmd = &get_cmd_midrawmatrix_html( \%PARAMS );
			$pbs->addCmd($cmd);
		}
		...
		if ( $domidrawdist == 1 ) {
			...
			my $todo =
"perl  $scriptdir/mi_create_dist_pvalues.pl -expfile $quantized_expfile_dna -profiles $profiles_dna -distrepfile $distfile_dna.rep -summaryfile $summaryfile_dna -motifdir $summaryfile_dna\_OUT -quantized $quantized -seqlen $seqlen_dna -rightlabel $rightlabel_dna -leftlabel $leftlabel_dna -outdistmatrix $outdistmatrix_dna -outoriematrix $outoriematrix_dna ";
			$pbs->addCmd($todo);
		...
		if ( ( $dodnacomp == 1 ) && ( $domidrawinteractions == 1 ) ) {
			...
			my $cmd = &get_cmd_midrawinteractions_html( \%PARAMS );
			$pbs->addCmd($cmd);
		}
		...
		if ( $domotifreport == 1 ) {
			...
			my $cmd =
"perl $scriptdir/generate_motif_occurrence_summary.pl -expfile $expfile_nodups_dna -summaryfile $summaryfile_dna -profiles $profiles_dna -rna $treatasrna  -pvmatrix $matrixfile_dna -columnfile $columnsfile_dna -distmatrix $outdistmatrix_dna -oriematrix $outoriematrix_dna -quantized $quantized ";
			$cmd .= " -gomotifs $gomofile_dna -goindex $goindex "
			  if ( defined($goindex) );
			$cmd .= " -profiles_orth $orth_profiles_dna "
			  if ( defined($fc_fasta2_dna) );
			$cmd .= " -mapping_orth $mapping_orth "
...
#    }
		...
		if ( $submit == 0 ) {
			...
			$jobid_dna = $pbs->submit;
			print "Submitted job $jobid_dna.\n";
		}
		if ( $dooptimizetowm == 1  && $dodnarna == 0) {
			my $pbs = PBS->new;
			...
			$pbs->setScriptName("intercept.script");

			$pbs->addCmd("date");

			$pbs->addCmd("echo \"DNA, optimize to WM\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
			...
			my $cmd = &get_cmd_optimizetowm_dna( \%PARAMS );
		...
		if ( $dooptimizetoextend == 1 && $dodnarna == 0) {
			my $pbs = PBS->new;
			...
			$pbs->setScriptName("extend.script");

			$pbs->addCmd("date");

			$pbs->addCmd("echo \"DNA, optimize to extend\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
				"fastafile_rna" => $fastafile_rna,
			...
			my $cmd = &get_cmd_optimizetoextend_dna( \%PARAMS );
...
#}
		...
		if ( $quantized == 0 ) {
			...
			my $cmd =
"perl $scriptdir/quantize_expression_vector.pl -expfile $expfile_nodups_rna -outfile $expfile_q_rna ";
			if ( defined($ebins) ) {
				$cmd .= " -ebins $ebins ";
			}
			if ( defined($divbins) ) {
...
"echo \"RNA, Step 2: skip seed optimization, use -motiffile_rna option instead.\""
			...
			$pbs->addCmd(
"perl $scriptdir/copy_seeds_to_optim.pl $motiffile_rna $optimfile_rna"
			);
		}

		if ( $doskipoptimization == 1 ) {
			$pbs->addCmd(
"echo \"RNA, Step 2: skip seed optimization, use seed file instead.\""
			);
			$pbs->addCmd(
"perl $scriptdir/copy_seeds_to_optim.pl $seedfile_rna $optimfile_rna"
			);
		}
		...
		if ( $dobinary == 1 ) {
			...
			my $cmd =
"perl $scriptdir/mi_create_overep_expfile.pl -repfile $motifrepfile_rna -expfile $quantized_expfile_rna -quantized $quantized -outexpfile $expfile_b_rna";
			$pbs->addCmd(
		...
		if ( ( $docons == 1 ) && ( defined($fc_fasta1_rna) ) ) {
			...
			my $cmd =
"perl $scriptdir/evaluate_motifs_conservation.pl -summaryfile $optimfile_rna -fasta1 $fc_fasta1_rna -fasta2 $fc_fasta2_rna -nbgenes $fc_nbgenes_rna -kmerfile $kmerfile_rna -outfile $consfile_rna -rna 1 ";
			if ( $shuffle_cons == 1 ) {
				$cmd .= " -shuffle 1";
			}
			$pbs->addCmd($cmd);
		...
		if ( $domisummarize == 1 ) {
			...
			my %PARAMS = (
				...
				"consfile_rna"   =>
				  ( defined($fc_fasta1_rna) ? $consfile_rna : undef ),
				"rna"               => 1,
				"quantized"         => $quantized,
				"minrobustness"     => $jn_t,
				"maxrobustness"     => $jn,
...
"echo \"RNA, Step 5.3: generate profiles for motifs in summary file.\""
			...
			my $cmd =
"perl $scriptdir/generate_motif_profiles.pl -fastafile $fastafile_rna -summaryfile $summaryfile_rna -outfile $profiles_rna -rna 1";
			$pbs->addCmd($cmd);

			if ( defined($fc_fasta2_rna) ) {
				$pbs->addCmd(
"echo \"RNA, Step 5.3 bis: Generate profiles for $fc_fasta2_rna.\""
				);
				my $cmd =
"perl $scriptdir/generate_motif_profiles.pl -fastafile $fc_fasta2_rna -summaryfile $summaryfile_rna -outfile $orth_profiles_rna -rna 1";
				$pbs->addCmd($cmd);
			}
		...
		if ( $dotargets == 1 ) {
			...
			my $cmd =
"perl $scriptdir/get_motif_targets.pl --matrixfile=$fullmatrixfile_rna --profiles=$profiles_rna --expfile=$quantized_expfile_rna --outfile=$targets_rna";
			$pbs->addCmd($cmd);
		...
		if ( $dohistograms == 1 ) {
			...
			my $cmd =
" perl $scriptdir/draw_position_histogram.pl  --profiles=$profiles_rna --expfile=$quantized_expfile_rna --outdir=$histogramdir_rna --targetfile=$targets_rna --rna=0 --seqlen=$seqlen_rna --bins=10 ";
			$pbs->addCmd($cmd);
...
"echo \"RNA, Step 5.6: generate genome-wide GO enrichments for motifs in summary file.\""
			...
			my $cmd =
"perl $scriptdir/motif_single_go_enrichment.pl --summaryfile=$summaryfile_rna --profiles=$profiles_rna --goindex=$goindex --gonames=$gonames --outgo=$gomofile_rna --outfile=$gomofile_full_rna --expfile=$quantized_expfile_rna --maxcatsize=$maxgocatsize ";
		...
		if ( $dodrawmotifmaps == 1 ) {
			...
			my $cmd =
"perl $scriptdir/mi_draw_all_pair_motif_maps.pl --expfile=$quantized_expfile_rna --profiles=$profiles_rna --rna=1 --summaryfile=$summaryfile_rna --rootdir=$scriptdir --fullmatrixfile=$fullmatrixfile_rna ";

			if ( defined($seqlen_rna) ) {
				$cmd .= " --seqlen=$seqlen_rna ";
			}
			else {
				$cmd .= " --fastafile=$fastafile_rna ";
			}
			if ( defined($leftlabel_rna) ) {
				$cmd .= " --leftlabel=$leftlabel_rna ";
			}
			if ( defined($rightlabel_rna) ) {
				$cmd .= " --rightlabel=$rightlabel_rna ";
			}
		...
		if ( ( $dogoclusters == 1 ) && ( defined($goindex) ) ) {
			...
			my $cmd =
"perl $scriptdir/clusters_go_enrichment.pl --clusters=$quantized_expfile_rna --goindex=$goindex --gonames=$gonames --N=-1 --outfile=$gofile_full_rna --outgo=$gofile_rna --maxcatsize=$maxgocatsize";
		...
		if ( $domotifnames == 1 ) {
			...
			if ( defined($acelibrary_rna) ) {
				...
				my $cmd =
"perl $scriptdir/annotate_motifs_using_ace_library.pl --acefiles=\"$acelibrary_rna\" --summaryfile=$summaryfile_rna --namefile=$namesfile_rna --rna=1 --outfile=$namesfile_rna_tmp";
				$pbs->addCmd($cmd);
...
"echo \"RNA, Step 6.8: identifying motifs that match micrornas.\""
				...
				my $cmd =
"perl $scriptdir/annotate_motifs_using_micrornas.pl --micrornas=$micrornas --summaryfile=$summaryfile_rna --namefile=$namesfile_rna > $namesfile_rna_tmp";
				$pbs->addCmd($cmd);
		...
		if ( ( $dornacomp == 1 ) && ( $domidrawmatrix == 1 ) ) {
			...
			my %PARAMS = (
				...
				"colmap"      => "$scriptdir/HEATMAPS/cmap2.txt",
				"namesfile"   => $namesfile_rna,
				"outfile"     => $summaryfile_rna . '.html'
			);
			$PARAMS{"clusterfile"} = $clusterfile_rna
			  if ( ( $sortmotifsbyphase == 0 ) && ( $domicombine >= 1 ) );
		...
		if ( $domidrawdist == 1 ) {
			...
			my $todo =
"perl  $scriptdir/mi_create_dist_pvalues.pl -expfile $quantized_expfile_rna -profiles $profiles_rna -distrepfile $distfile_rna.rep -summaryfile $summaryfile_rna -motifdir $summaryfile_rna\_OUT -quantized $quantized -seqlen $seqlen_rna -rightlabel $rightlabel_rna -leftlabel $leftlabel_rna -outdistmatrix $outdistmatrix_rna  -outoriematrix $outoriematrix_rna ";
			$pbs->addCmd($todo);
		...
		if ( $domotifreport == 1 ) {
			...
			my $cmd =
"perl $scriptdir/generate_motif_occurrence_summary.pl -expfile $expfile_nodups_rna -summaryfile $summaryfile_rna -profiles $profiles_rna -rna 1  -pvmatrix $matrixfile_rna -columnfile $columnsfile_rna -distmatrix $outdistmatrix_rna -oriematrix $outoriematrix_rna -quantized $quantized ";
			$cmd .= " -gomotifs $gomofile_rna -goindex $goindex "
			  if ( defined($goindex) );
			$cmd .= " -profiles_orth $orth_profiles_rna "
			  if ( defined($fc_fasta2_rna) );
			$cmd .= " -mapping_orth $mapping_orth "
			  if ( defined($mapping_orth) );
...
#    }
		...
		if ( $submit == 0 ) {
			...
			$jobid_rna = $pbs->submit;
			print "Submitted job $jobid_rna.\n";
		}
		...
		if ( $dooptimizetowm == 1 && $dodnarna == 0) {
			...
			$pbs->addCmd("echo \"RNA, optimize to WM\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
				"fastafile_rna" => $fastafile_rna,
			...
			my $cmd = &get_cmd_optimizetowm_rna( \%PARAMS );
		...
		if ( $dooptimizetoextend == 1 && $dodnarna == 0 ) {
			...
			$pbs->addCmd("echo \"RNA, optimize to extend\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
				"fastafile_rna" => $fastafile_rna,
			...
			my $cmd = &get_cmd_optimizetoextend_rna( \%PARAMS );
	...
	if ( $dodnarna == 1 ) {
		...
		#
		die
"No DNA or no RNA sequence data. Perhaps you need to download the relevant 
species-specific data file from http://tavazoielab.princeton.edu/FIRE.\n"
...
"CNTM=`perl $scriptdir/count_lines.pl $seedfile_dna $seedfile_rna`; if [ \$CNTM = 0 ]; then echo \"No DNA or RNA motifs. Exiting now.\"; exit; fi"
		...
		if ( $quantized == 0 ) {
			...
			#
			my $cmd =
"perl $scriptdir/intersect_expression_vectors.pl -expfile1 $expfile_nodups_dna -expfile2 $expfile_nodups_rna -outfile $expfile_nodups_dna_rna";
			$pbs->addCmd($cmd);
			...
			if ( $quantized == 0 ) {
				...
				my $cmd =
"perl $scriptdir/quantize_expression_vector.pl -expfile $expfile_nodups_dna_rna -outfile $expfile_q_dna_rna ";
				if ( defined($ebins) ) {
					$cmd .= " -ebins $ebins ";
				}
				if ( defined($divbins) ) {
			...
			if ( $domisignif == 1 ) {
				...
				$pbs->addCmd(
"echo \"DNA/RNA, Step 3: re-generating motif reports for DNA.\""
				);
				my %PARAMS = (
					"expfile"      => $expfile_nodups_dna_rna,
					"quantized"    => $quantized,
					"fastafile"    => $fastafile_dna,
					"rna"          => 1,
					"ebins"        => $ebins,
				...
				$pbs->addCmd(
"echo \"DNA/RNA, Step 3: re-generating motif reports for RNA.\""
				);
				my %PARAMS = (
					"expfile"      => $expfile_nodups_dna_rna,
					"quantized"    => $quantized,
		...
		if ( $domisummarize == 1 ) {
			...
			my %PARAMS = (
				...
				"signiffile_dna"   => $signiffile_dna,
				"motifrepfile_dna" => (
					$quantized == 0 ? "$new_signiffile_dna.motifs.rep" : undef
				),
				"distfile_dna" => $distfile_dna,
				"consfile_dna" =>
				  ( defined($fc_fasta1_dna) ? $consfile_dna : undef ),
				...
				"signiffile_rna"   => $signiffile_rna,
				"motifrepfile_rna" => (
					$quantized == 0 ? "$new_signiffile_rna.motifs.rep" : undef
				),
				"distfile_rna" => $distfile_rna,
				"consfile_rna" =>
				  ( defined($fc_fasta1_rna) ? $consfile_rna : undef ),
		...
		if ( ( $domicombine == 1 ) && ( $sortmotifsbyphase == 0 ) ) {
			...
			my %PARAMS = (
				"expfile"          => $expfile_b_summary_dna_rna,
				"quantized"        => 1,
				"fastafile_dna"    => $fastafile_dna,
				"fastafile_rna"    => $fastafile_rna,
				"summaryfile"      => $summaryfile_dna_rna,
...
#        }
		...
		if ( ( $dogoclusters == 1 ) && ( defined($goindex) ) ) {
			...
			my $cmd =
"perl $scriptdir/clusters_go_enrichment.pl --clusters=$quantized_expfile_dna_rna --goindex=$goindex --gonames=$gonames --N=-1 --outfile=$gofile_full_dna_rna --outgo=$gofile_dna_rna --maxcatsize=$maxgocatsize ";
		...
		#
		$pbs->addCmd(
"perl $scriptdir/cat_ifexist.pl $namesfile_dna $namesfile_rna > $namesfile_dna_rna"
		);
		...
		#
		$pbs->addCmd(
"perl $scriptdir/cat_ifexist.pl $gomofile_dna $gomofile_rna > $gomofile_dna_rna"
		);
		...
		if ( $domidrawmatrix == 1 ) {
			...
			my %PARAMS = (
				...
				"colmap"    => "$scriptdir/HEATMAPS/cmap2.txt",
				"namesfile" => $namesfile_dna_rna,
				"outfile"   => $summaryfile_dna_rna . '.html'
			);

	#$PARAMS{"clusterfile"} = $clusterfile_dna_rna if ($sortmotifsbyphase == 0);
			$PARAMS{"clusterfile"} = $clusterfile_dna_rna
			  if ( ( $sortmotifsbyphase == 0 ) && ( $domicombine >= 1 ) );
			...
			$pbs->addCmd(
				"echo \"DNA,RNA, Step 7: draw matrix density figure.\"");
			...
			my %PARAMS = (
				...
				"namesfile"   => $namesfile_dna_rna,
				"outfile"     => $densityfile_dna_rna . '.html'
			);
	...
	#$PARAMS{"clusterfile"} = $clusterfile_dna_rna if ($sortmotifsbyphase == 0);
		...
		if ( $dodrawmotifmaps == 1 ) {
			...
			#
			$pbs->addCmd(
"echo \"DNA/RNA, draw motif maps for single motifs and co-localizing pairs\""
			);
			mkdir "$mimatrixfile_dna_rna\_OUT"
			  if ( !-e "$mimatrixfile_dna_rna\_OUT" );

			#
			# do DNA first
			#
			my $cmd =
"perl $scriptdir/mi_draw_all_pair_motif_maps.pl --expfile=$quantized_expfile_dna_rna --profiles=$profiles_dna --seqlen=$seqlen_dna --rna=0 --summaryfile=$summaryfile_dna_rna --leftlabel=$leftlabel_dna --rightlabel=$rightlabel_dna --rootdir=$scriptdir --mimatrixfile=$mimatrixfile_dna_rna --fullmatrixfile=$fullmatrixfile_dna_rna "
			  ;    # > $mimatrixfile_dna\_OUT/index.html";
			$pbs->addCmd($cmd);
			...
			#
			my $cmd =
"perl $scriptdir/mi_draw_all_pair_motif_maps.pl --expfile=$quantized_expfile_dna_rna --profiles=$profiles_rna --seqlen=$seqlen_rna --rna=1 --summaryfile=$summaryfile_dna_rna --leftlabel=$leftlabel_rna --rightlabel=$rightlabel_rna --rootdir=$scriptdir --mimatrixfile=$mimatrixfile_dna_rna --fullmatrixfile=$fullmatrixfile_dna_rna "
			  ;    # > $mimatrixfile_dna\_OUT/index.html";
			$pbs->addCmd($cmd);
		...
		if ( $submit == 0 ) {
			...
			$jobid_dnarna = $pbs->submit;
			print "Submitted job $jobid_dnarna.\n";
		...
		if ( $dooptimizetowm == 1 ) {
			...
			$pbs->addCmd("echo \"DNARNA, optimize to WM\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
				"fastafile_rna" => $fastafile_rna,
			...
			my $cmd = &get_cmd_optimizetowm_dnarna( \%PARAMS );
		...
		if ( $dooptimizetoextend == 1 ) {
			...
			$pbs->addCmd("echo \"DNARNA, optimize to extend\"");
			my %PARAMS = (
				"expfile"       => $expfile,
				"fastafile_dna" => $fastafile_dna,
				"fastafile_rna" => $fastafile_rna,
			...
			my $cmd = &get_cmd_optimizetoextend_dnarna( \%PARAMS );
...
sub get_cmd_removedups {
	...
	my $todo =
"perl $scriptdir/remove_homologous_sequences_withseed.pl -expfile $p->{expfile} -quantized $p->{quantized} -fastafile $p->{fastafile} -outfile $p->{outfile}";
...
sub get_cmd_mifind {
	...
	my $todo =
"$progdir/mi_find -expfile $p->{expfile} -quantized $p->{quantized} -fastafile $p->{fastafile} -outfile $p->{seedfile} -report 1 -docor 1 -outprm 1 -shuffle $p->{shuffle} -jn 0 -jn_t 0 -verbose 1 -rna $p->{rna}";
...
sub get_cmd_mioptimize {
	...
	if ( $p->{optimslow} == 0 ) {
		...
		$todo =
"$progdir/mi_optimize -expfile $p->{expfile} -quantized $p->{quantized} -fastafile $p->{fastafile} -outfile $p->{optimfile} -kmerfile $p->{seedfile} -shuffle $p->{shuffle} -report 0 -log 1 -rna $p->{rna} -verbose 0 -mask 0 -minr $p->{minr} -maxfreq $p->{maxfreq}";

	}
	else {

		$todo =
"$progdir/mi_optimize_slow -expfile $p->{expfile} -quantized $p->{quantized} -fastafile $p->{fastafile} -outfile $p->{optimfile} -kmerfile $p->{seedfile} -shuffle $p->{shuffle} -report 0 -log 1 -rna $p->{rna}";
...
sub get_cmd_misignif {
	...
	my $todo =
"$progdir/mi_signif -expfile $p->{expfile} -quantized $p->{quantized} -fastafile $p->{fastafile} -motiffile $p->{optimfile} -shuffle $p->{shuffle} -optimout 1 -outfile $p->{signiffile} -rna $p->{rna} -jn $p->{jn} -jn_t $p->{jn_t} -jn_f $p->{jn_f} ";
...
# my ($expfile, $quantized, $fastafile, $rna, $optimfile, $shuffle, $mbins_interval, $donbcopies, $outfile) = @_;
	...
	my $todo =
"$progdir/mi_dist -expfile $p->{expfile} -expfile_b $p->{expfile_b} -quantized $p->{quantized} -fastafile $p->{fastafile} -motiffile $p->{optimfile} -shuffle $p->{shuffle} -outfile $p->{distfile} -mbins_interval $p->{mbins_interval} -rna $p->{rna} -donbcopies $p->{donbcopies}";
...
#    $outsummary, $outmatrix, $outcolumns, $outgoodcolumns, $outnames, $outexpfile) = @_;
	...
	if ( ( $p->{rna} == 0 ) || ( $p->{rna} == 2 ) ) {
		$todo .=
" -optimfile_dna $p->{optimfile_dna} -statfile_dna $p->{signiffile_dna} -distfile_dna $p->{distfile_dna} ";
	}

	if ( ( $p->{rna} == 1 ) || ( $p->{rna} == 2 ) ) {
		$todo .=
" -optimfile_rna $p->{optimfile_rna} -statfile_rna $p->{signiffile_rna} -distfile_rna $p->{distfile_rna} ";
	}

	if ( $p->{rna} == 2 ) {
		$todo .=
" -expfile_dna $p->{expfile_dna} -expfile_rna $p->{expfile_rna} -outexpfile $p->{outexpfile} ";
	}

	$todo .=
" -quantized $p->{quantized} -outmatrix $p->{matrixfile} -outfullmatrix $p->{fullmatrixfile} -outdensitymatrix $p->{densityfile} -outcolumns $p->{columnsfile} -outgoodcolumns $p->{signifcolumnsfile} -outnames $p->{namesfile} -outsummary $p->{summaryfile} -removecols_exp 0 -minrobustness $p->{minrobustness} -outlevel $p->{outlevel} -maxrobustness $p->{maxrobustness} ";
...
sub get_cmd_micombine {
	...
	$todo .=
" -summaryfile $p->{summaryfile} -quantized $p->{quantized} -outfile $p->{clusterfile} -outmimatrix $p->{mimatrixfile} -outfullmimatrix $p->{fullmimatrixfile} -poscor $p->{poscor} -doallstats $p->{doallstats} ";
...
sub get_cmd_midrawmatrix {
	...
	my $todo =
"perl $scriptdir/mi_draw_matrix.pl --expfile=$p->{expfile} --matfile=$p->{matrixfile} --summaryfile=$p->{summaryfile} --columnsfile=$p->{columnsfile} --ps2pdf=$p->{ps2pdf} --every=$p->{every} --quantized=$p->{quantized} --motifnames=$p->{namesfile} --ybase=250";
...
sub get_cmd_midrawinteractions {
	...
	my $todo =
"perl $scriptdir/mi_draw_interaction_matrix.pl -summaryfile $p->{summaryfile} -matrixfile $p->{fullmimatrixfile} -resmatrixfile $p->{mimatrixfile} -motifdir $p->{motifdir} ";
	if ( defined( $p->{clusterfile} ) ) {
		$todo .= " -clustfile $p->{clusterfile} ";
	}
...
sub get_cmd_midrawmatrix_html {
	...
	my $todo =
"perl $scriptdir/mi_draw_matrix_html.pl --expfile=$p->{expfile} --matfile=$p->{matrixfile} --summaryfile=$p->{summaryfile} --columnsfile=$p->{columnsfile} --ps2pdf=$p->{ps2pdf} --every=$p->{every} --quantized=$p->{quantized} --motifnames=$p->{namesfile} --ybase=250";
...
sub get_cmd_midrawinteractions_html {
	...
	my $todo =
"perl $scriptdir/mi_draw_interaction_matrix_html.pl -summaryfile $p->{summaryfile} -matrixfile $p->{fullmimatrixfile} -resmatrixfile $p->{mimatrixfile} -motifdir $p->{motifdir} ";
	if ( defined( $p->{clusterfile} ) ) {
		$todo .= " -clustfile $p->{clusterfile} ";
	}
	if ( defined( $p->{orderfile} ) ) {
...
sub get_cmd_optimizetowm_dnarna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_to_WM_using_grid_dnarna.pl --expfile=$p->{expfile} --fastafile_dna=$p->{fastafile_dna} --fastafile_rna=$p->{fastafile_rna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub get_cmd_optimizetoextend_dnarna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_using_grid_dnarna.pl --expfile=$p->{expfile} --fastafile_dna=$p->{fastafile_dna} --fastafile_rna=$p->{fastafile_rna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub get_cmd_optimizetoextend_dna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_using_grid_dna.pl --expfile=$p->{expfile} --fastafile_dna=$p->{fastafile_dna} --fastafile_rna=$p->{fastafile_rna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub get_cmd_optimizetoextend_rna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_using_grid_rna.pl --expfile=$p->{expfile} --fastafile_dna=$p->{fastafile_dna} --fastafile_rna=$p->{fastafile_rna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub get_cmd_optimizetowm_dna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_to_WM_using_grid_dna.pl --expfile=$p->{expfile} --fastafile_dna=$p->{fastafile_dna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub get_cmd_optimizetowm_rna {
	...
	my $todo =
"perl $morescriptdir/optimize_each_motif_to_WM_using_grid_rna.pl --expfile=$p->{expfile} --fastafile_rna=$p->{fastafile_rna} --ebins=$p->{ebins} --quantized=$p->{quantized} --submit=1 ";
...
sub check_input_file {
	...
	foreach my $r (@$a_ref) {
		if ( defined( $H{ $r->[0] } ) ) {
			print
"Your files contains multiple rows with the same gene id. Please correct that before applying FIRE.\n";
			return 0;
		}
		$H{ $r->[0] }++;
		$V{ $r->[1] } = 1;
	...
	if ( ( $exptype eq 'discrete' ) && ( scalar(@v) != $max + 1 ) ) {
		...
		my $n2 = $max + 1;
		die
"Problem. Your discrete vector is missing some symbols ($n1 != $n2).\n";
		return 0;
	}
